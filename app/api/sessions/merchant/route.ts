...
export async function GET(request: NextRequest) {
  try {
    ...
    // Kullanıcının merchantId'sine göre oturumları getir
    const sessions = await Session.find({ merchantId: claims.merchantId, status: { $ne: 'disconnected' } });

    return NextResponse.json(sessions);
  } catch (error: any) {
    console.error('Merchant oturumları getirme hatası:', error);
    return NextResponse.json(
